...
<body>
    ...
    <main>
      <table class="table table-responsive-sm table-hover caption-top">
        <caption><h1 class="header2">Turnyrinė komandų lentelė</h1></caption>
        <thead>
          <tr style="background-color: #f5f5f4;">
            <th scope="col"></th>
            <th scope="col" class="fotke"></th>
            <th scope="col" style="font-size: 25px;">Komanda</th>
            <th scope="col" style="text-align: center; font-size: 25px;">Rungtynės</th>
            <th scope="col" style="text-align: center; font-size: 25px;">Pergalės</th>
            <th scope="col" style="text-align: center; font-size: 25px;">Pralaimėjimai</th>
          </tr>
        </thead>
        <tbody class="tbody"></tbody>
      <script>
        let http = new XMLHttpRequest();
          http.open('get', 'scrapinam/LENTELE.json', true);
          http.send();
                ...
                for(let item of products){
                  output += `
                      <tr>
                        ...
                        <td class="fotke">
                            <a>
                              <img src="${item.Nuotrauka}"></img>
                            </a>
                            <div>
                                <td style="font-weight: bold">${item.Komanda}</td>
                            </div>
                        </td>
                        <td style="text-align: center; color:black; font-weight: bold;">
                            <span style="background-color: #f5f5f4; padding: 3%; border-radius: 5px">${item.Rng}</span>
                        </td>
                        <td style="text-align: center; color:#15803d; font-weight: bold;">
                            <span style="background-color: #dcfce7; padding: 3%; border-radius: 5px">${item.Per}</span>
                        </td>
                        <td style="text-align: center; color:#991b1b; font-weight: bold;">
                            <span style="background-color: #fee2e2; padding: 2%; border-radius: 5px">${item.Pr}</span>
                        </td>
                      </tr>
                  `;
                }
